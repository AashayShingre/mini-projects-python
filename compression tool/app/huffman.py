...
@functools.total_ordering # for heapify
class HuffmanNode: 
    freq: int 
    val: Optional[str] 
...
def buildTree(freqTable: dict[str, int]) -> HuffmanNode: 
    ...
    while len(minHeap) > 1: 
        ...
        parentNode = HuffmanNode(leftNode.freq + rightNode.freq, None, leftNode, rightNode)
...
def updateHuffmanTable(huffmanTable: dict[str, str], node: HuffmanNode, prefix: str): 
    ...
    if node.val: 
        ...
        if node.left and node.right: 
            ...
            raise ValueError(f"updateHuffmanTable: Unknown error -> both left and right now expected {node}")
...
def decompressData(strLen: int, tree: Optional[HuffmanNode] = None, bits: Optional[Iterable] = None, char: Optional[str] = None) -> str:
    if bits and tree: 
        resultChars = []
        ref = tree 
        ...
        for bit in bits:
            if not ref: 
                raise ValueError(f"decompressData: Unexpected set of bits for tree {tree}") 
            ref = ref.right if bit else ref.left
            if ref and ref.val: # encountered leaf 
                resultChars.append(ref.val)
                if len(resultChars) == strLen: break
