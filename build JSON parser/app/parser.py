...
class Parser: 
    ...
    def parse(self):  
        if self.n > 0 and self.tokens[0][0] == tokenType.LEFT_CURL:
            self.seek = 1
    ...
    def getObject(self): 
        ...
        while self.seek < self.n:
            ...
            if (c := self.tokens[self.seek][0]) == tokenType.COMMA: 
                ...
                self.raiseInvalid(f"Unexpected token at {self.seek} ({c}). , or {'}'} expected") 
    ...
    def getValue(self): 
        ...
        match self.tokens[self.seek]: 
            case (tokenType.STRING | tokenType.NUMBER | tokenType.BOOLEAN | tokenType.NULL, val): 
                self.seek += 1
                return val
